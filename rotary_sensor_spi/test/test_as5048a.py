...
class TestAS5048A(TestCase):
    ...
    def test_calc_even_parity(self):
        ...
        p = 1
        for i in range(0, 15):
            self.assertEqual(AS5048A.calc_even_parity(0x7fff >> i), p)
            p = 1 - p

        # Random tests
        self.assertEqual(AS5048A.calc_even_parity(0b000010000010000), 0)
        self.assertEqual(AS5048A.calc_even_parity(0b000010010010000), 1)
        self.assertEqual(AS5048A.calc_even_parity(0b001010011000101), 0)
        self.assertEqual(AS5048A.calc_even_parity(0b101100101001011), 0)
        self.assertEqual(AS5048A.calc_even_parity(0b011001100010000), 1)
        self.assertEqual(AS5048A.calc_even_parity(0b11110000010000), 1)
        self.assertEqual(AS5048A.calc_even_parity(0b001011011110100), 0)
        self.assertEqual(AS5048A.calc_even_parity(0b110000110010111), 0)
        self.assertEqual(AS5048A.calc_even_parity(0b110111100110101), 0)
        self.assertEqual(AS5048A.calc_even_parity(0b101010101010101), 0)
        self.assertEqual(AS5048A.calc_even_parity(0b010101010101010), 1)

    def test_calc_angle(self):
        self.assertEqual(AS5048A.calc_angle(0), 0)
        self.assertEqual(AS5048A.calc_angle(0x1000), math.pi/2)
    ...
    def test_read(self, mock_spi):
        ...
        with mock.patch.object(AS5048A, 'transfer', return_value=0x3fff) as mock_transfer:
            as5048.read(AS5048A.ADDR_NOP)
            ...
            as5048.read(AS5048A.ADDR_ANGLE)
            self.assertEqual(mock_transfer.call_count, 2)
        ...
        response = as5048.read(AS5048A.ADDR_ANGLE)
        # Check calls
    ...
    def test_write(self, mock_spi):
        ...
        response = as5048.write(AS5048A.ADDR_ZERO_POSITION_HI, 0x0004)
        # Check calls
